# CESI - Shear valve test program
# Possible to drive motor and communicates with the absolute encoder and reports the rotational position of the axle
# It also samples the scope ch1 for potentiometer position. Used to find if the absolute encoder contains dead zones
...
def InitI2C (speed, SCL, SDA):
    ...
    dwf.FDwfDigitalI2cRateSet(hdwf, c_double(speed)) # speed
    dwf.FDwfDigitalI2cSclSet(hdwf, c_int(SCL)) # SCL = DIO-x
    dwf.FDwfDigitalI2cSdaSet(hdwf, c_int(SDA)) # SDA = DIO-y
...
# initiate I2C on SCL = DIO-14, SDA = DIO-7 at speed 400kHz 
InitI2C (4e5, 14, 7)
